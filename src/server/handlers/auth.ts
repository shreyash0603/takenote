...
export default {
  ...
  callback: async (request: Request, response: Response) => {
    ...
    try {
      ...
      // In development, frontend runs on :3000 via webpack dev server.
      // In production, Express serves the built app from /dist.
      response.redirect(isProduction ? '/app' : 'http://localhost:3000/app')
    } catch (error) {
      console.log(error) // eslint-disable-line
      // Redirect to landing page for the active runtime target.
      response.redirect(isProduction ? '/' : 'http://localhost:3000/')
    }
  },
